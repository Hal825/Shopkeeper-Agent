...
async def merge_retrieved_info(
    ...
    retrieved_column_infos = state.get("retrieved_column_infos", [])
    retrieved_metric_infos = state.get("retrieved_metric_infos", [])
    ...
    # 每个指标有一个relevant_columns属性，它是一个列表，包含该指标计算所依赖的底层字段的ID
    # （例如["fact_order.order_amount"]）。
    ...
    # column_info	 table_id	     字典状态
# 1	order_amount	"fact_order"	{"fact_order": []} → 追加后 → {"fact_order": [order_amount]}
    ...
    for table_id in table_to_columns_map.keys():
        ...
        for key_column in key_columns:
            # 这里不能直接用：if key_column not in table_to_columns_map[table_id]:
            if key_column.id not in column_ids:
                table_to_columns_map[table_id].append(key_column)
            # 因为key_column是对象。直接比较对象时，很容易变成比较对象地址，
            # 而不是比较字段ID。这里更稳的做法是先拿到当前表已经存在的字段ID：
    ...
    # .items()：是Python字典的方法，返回一个包含所有键值对的可迭代对象。
