...
class MetaMySQLRepository:
    ...
    # row代表查询结果集中的一行数据，它是一个SQLAlchemy提供的Row对象（或Mapping视图）。
    # result.mappings().fetchall()返回一个列表，每个元素是一个类似字典的行对象，可以通过列名（如"id"、"name"、"table_id"、"role"等）访问对应的值。
    # dict(row)将这个行对象转换为一个普通的字典，例如：
    # {
